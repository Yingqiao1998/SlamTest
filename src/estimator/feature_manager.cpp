...
bool FeatureManager::addFeatureCheckParallax(
  ...
  if (parallax_num == 0) {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("feature_manager"), "parallax_sum: %lf, parallax_num: %d",
                 parallax_sum, parallax_num);
    RCLCPP_DEBUG(rclcpp::get_logger("feature_manager"), "current parallax: %lf",
...
void FeatureManager::triangulatePoint(Eigen::Matrix<double, 3, 4> &Pose0,
                                      Eigen::Matrix<double, 3, 4> &Pose1, Eigen::Vector2d &point0,
...
void FeatureManager::triangulate(int frameCnt, Vector3d Ps[], Matrix3d Rs[], Vector3d tic[],
                                 Matrix3d ric[]) {
  for (auto &it_per_id : feature) {
    if (it_per_id.estimated_depth > 0)
      continue;
