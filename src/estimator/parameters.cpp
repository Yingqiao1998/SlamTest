...
template <typename T> T readParam(rclcpp::Node::SharedPtr node, std::string name) {
  T ans;
...
void readParameters(std::string config_file) {
  ...
  if (fh == NULL) {
    RCLCPP_WARN(rclcpp::get_logger("parameters"),
                "config_file dosen't exist; wrong config_file path");
    return;
  }
  fclose(fh);
  ...
  RCLCPP_INFO(rclcpp::get_logger("parameters"), "ROW: %d COL: %d ", ROW, COL);
