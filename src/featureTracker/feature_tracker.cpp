...
FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1) {
  ...
  // 检查相机是否初始化
  if (m_camera.empty()) {
    RCLCPP_ERROR(rclcpp::get_logger("feature_tracker"), "m_camera is empty! Camera not initialized!");
    map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> emptyFrame;
    return emptyFrame;
  }
  if (m_camera[0] == nullptr) {
    RCLCPP_ERROR(rclcpp::get_logger("feature_tracker"), "m_camera[0] is nullptr!");
    map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> emptyFrame;
    return emptyFrame;
  }
  
  RCLCPP_DEBUG(rclcpp::get_logger("feature_tracker"), "trackImage called with image size: %dx%d", _img.cols, _img.rows);
  ...
  if (prev_pts.size() > 0) {
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("feature_tracker"), "temporal optical flow costs: %fms",
                 t_o.toc());
    //printf("track cnt %d\n", (int)ids.size());
  }
  ...
  if (1) {
    ...
    if (n_max_cnt > 0) {
      ...
      n_pts.clear();
    RCLCPP_DEBUG(rclcpp::get_logger("feature_tracker"), "detect feature costs: %f ms", t_t.toc());
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("feature_tracker"), "About to undistort %lu points", cur_pts.size());
  
  cur_un_pts = undistortedPts(cur_pts, m_camera[0]);
  
  RCLCPP_DEBUG(rclcpp::get_logger("feature_tracker"), "Undistortion completed, got %lu points", cur_un_pts.size());
...
void FeatureTracker::rejectWithF() {
  if (cur_pts.size() >= 8) {
    ...
    reduceVector(ids, status);
    reduceVector(track_cnt, status);
    RCLCPP_DEBUG(rclcpp::get_logger("feature_tracker"), "FM ransac: %d -> %lu: %f", size_a,
                 cur_pts.size(), 1.0 * cur_pts.size() / size_a);
...
void FeatureTracker::readIntrinsicParameter(const vector<string> &calib_file) {
  for (size_t i = 0; i < calib_file.size(); i++) {
    RCLCPP_INFO(rclcpp::get_logger("feature_tracker"), "reading paramerter of camera %s",
                calib_file[i].c_str());
...
vector<cv::Point2f> FeatureTracker::undistortedPts(vector<cv::Point2f> &pts,
  ...
  RCLCPP_DEBUG(rclcpp::get_logger("feature_tracker"), "undistortedPts: processing %lu points", pts.size());
  
  if (cam == nullptr) {
    RCLCPP_ERROR(rclcpp::get_logger("feature_tracker"), "undistortedPts: camera pointer is null!");
    return un_pts;
  }
  ...
  for (unsigned int i = 0; i < pts.size(); i++) {
    ...
    try {
      ...
      // 检查除零
      if (std::abs(b.z()) < 1e-10) {
        RCLCPP_WARN(rclcpp::get_logger("feature_tracker"), "undistortedPts: b.z() is too small: %f", b.z());
        continue;
      }
      ...
      RCLCPP_ERROR(rclcpp::get_logger("feature_tracker"), "undistortedPts: exception in liftProjective: %s", e.what());
      continue;
    }
  }
  
  RCLCPP_DEBUG(rclcpp::get_logger("feature_tracker"), "undistortedPts: completed, output %lu points", un_pts.size());
